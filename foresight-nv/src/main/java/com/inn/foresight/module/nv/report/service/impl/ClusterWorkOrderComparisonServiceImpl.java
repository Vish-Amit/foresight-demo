...
import com.inn.foresight.module.nv.report.service.IClusterWorkOrderComparisonService;
 ...
 */
@Service("ClusterWorkOrderComparisonServiceImpl")
public class ClusterWorkOrderComparisonServiceImpl implements IClusterWorkOrderComparisonService{

	/** The logger. */
	private Logger logger = LogManager.getLogger(ClusterWorkOrderComparisonServiceImpl.class);
	 ...
	 */
	public ClusterWorkOrderComparisonServiceImpl() {
		super();
	}
	...
	private CustomerComplaintSubWrapper getKpiSummaryDataListForWorkOrderID(List<Integer> workorderId, Boolean isPre,
		...
		try {
			...
			if (map != null) {
				String summary=reportService.getSummaryDataForWoIds(workorderId,
						map.get(QMDLConstant.RECIPE), map.get(QMDLConstant.OPERATOR));
				String[] summaryData = summary.replaceAll("\\]", ReportConstants.BLANK_STRING).replaceAll("\\[", ReportConstants.BLANK_STRING).split(ReportConstants.COMMA);
				if (isPre) {
					wrapper.setPreList(setSummaryData(summaryData));
				} else {
					wrapper.setPostList(setSummaryData(summaryData));
				}
			}
		} catch (Exception e) {
	 ...
	 * @return the list
	 */
	private List<AvrageKpiWrapper> setSummaryData(String[] summaryData) {
		AvrageKpiWrapper wrapper=new AvrageKpiWrapper();
		List<AvrageKpiWrapper>list=new ArrayList<>();
		try{
			if(summaryData[DriveHeaderConstants.SUMMARY_DL_INDEX] != null && !summaryData[DriveHeaderConstants.SUMMARY_DL_INDEX].isEmpty()){
				wrapper.setAvgDl(ReportUtil.parseToFixedDecimalPlace(Double.parseDouble(summaryData[DriveHeaderConstants.SUMMARY_DL_INDEX]),ReportConstants.INDEX_THREE));
			}
			if(summaryData[DriveHeaderConstants.SUMMARY_SINR_INDEX] != null && !summaryData[DriveHeaderConstants.SUMMARY_SINR_INDEX].isEmpty()){
				wrapper.setAvgSinr(ReportUtil.parseToFixedDecimalPlace(Double.parseDouble(summaryData[DriveHeaderConstants.SUMMARY_SINR_INDEX]),ReportConstants.INDEX_THREE));
			}
			if(summaryData[DriveHeaderConstants.SUMMARY_RSRP_INDEX] != null && !summaryData[DriveHeaderConstants.SUMMARY_RSRP_INDEX].isEmpty()){
				wrapper.setAvgRsrp(ReportUtil.parseToFixedDecimalPlace(Double.parseDouble(summaryData[DriveHeaderConstants.SUMMARY_RSRP_INDEX]),ReportConstants.INDEX_THREE));
			}
			if(summaryData[DriveHeaderConstants.SUMMARY_UL_INDEX] != null && !summaryData[DriveHeaderConstants.SUMMARY_UL_INDEX].isEmpty()){
				wrapper.setAvgUl(ReportUtil.parseToFixedDecimalPlace(Double.parseDouble(summaryData[DriveHeaderConstants.SUMMARY_UL_INDEX]),ReportConstants.INDEX_THREE));
			}
			list.add(wrapper);
		} catch (Exception e) {
	...
	private List<KPIImgDataWrapper> getKPiImageDataListForWorkOrderID(List<SiteInformationWrapper> listOfSites, List<Integer> workorderIds, List<KPIImgDataWrapper> listofKpiImg, Boolean isPre, List<List<List<List<Double>>>> boundaryData, List<List<List<Double>>> allCustomRoutes) {
		...
		try {
			...
			if(map!=null){
				...
				List<KPIWrapper> kpiList = reportService.getKPiStatsDataList(workorderIds,map,DriveHeaderConstants.getKpiIndexMap(),ReportConstants.PREPOST);
				DriveImageWrapper driveImageWrapper = getDriveImageWrapperDataForImageGenearation(workorderIds,map,kpiList,boundaryData,allCustomRoutes,listOfSites);
				String summaryData = reportService.getSummaryDataForWoIds(workorderIds,map.get(QMDLConstant.RECIPE), map.get(QMDLConstant.OPERATOR));
				return getListofImages(driveImageWrapper,kpiList,listofKpiImg, workorderIds,isPre,summaryData);
			}
		} catch (Exception e) {
	 ...
	 */
	private List<KPIImgDataWrapper> getListofImages(DriveImageWrapper driveImageWrapper, List<KPIWrapper> kpiList, List<KPIImgDataWrapper> listofKpiImg, List<Integer> workorderIds, Boolean isPre, String summaryData) {
		logger.info("Inside method getListofImages for workOrderIds {} ",workorderIds);
		try {
			...
			kpiList.stream().forEach(kpiWrapperObj->{
				try {
					if (kpiWrapperObj.getIndexKPI().toString()
							.equalsIgnoreCase(DriveHeaderConstants.INDEX_PCI.toString())
							|| kpiWrapperObj.getIndexKPI().toString()
	 ...
	 * @param summaryData 
	 */
	private void setDataInWrapper(Boolean isPre, KPIWrapper kpiWrapperObj, List<KPIImgDataWrapper> listofKpiImg, Map<String, BufferedImage> driveImageMap, Map<String, BufferedImage> legendImageMap, String summaryData) {
		...
		try {
			if(!isPre){
				...
				if(is!=null){
					...
					kpiImgWrapper.setAvgBeforeKpi(getAverageValueFromSummaryData(summaryData,getAvgValueIndexofSummary(kpiWrapperObj.getKpiName())));
				...
				if(kpiImgWrapperList!=null && !kpiImgWrapperList.isEmpty()){
					...
					kpiImgWrapper.setAvgAfterKpi(getAverageValueFromSummaryData(summaryData,getAvgValueIndexofSummary(kpiImgWrapper.getKpiName().replaceAll(ReportConstants.SPACE,ReportConstants.UNDERSCORE))));
					if(kpiWrapperObj.getKpiName().equalsIgnoreCase(ReportConstants.ROUTE)){
						logger.info("Setting Route info");
						kpiImgWrapper.setBeforImg(getInputStreamFromBufferedImage(driveImageMap.get("TERRAIN0")));
					}
	...
	private Integer getAvgValueIndexofSummary(String kpiName) {
		switch (kpiName) {
		...
		case ReportConstants.HTTP_UL_RSRP:
			return DriveHeaderConstants.SUMMARY_RSRP_INDEX;
		...
		case ReportConstants.HTTP_UL_SINR:
			return DriveHeaderConstants.SUMMARY_SINR_INDEX;
		case ReportConstants.UL:
		case ReportConstants.HTTP_UL_THROUGHPUT:
		case ReportConstants.FTP_UL_THROUGHPUT:
			return DriveHeaderConstants.SUMMARY_UL_INDEX;
		case ReportConstants.DL:
		case ReportConstants.HTTP_DL_THROUGHPUT:
		case ReportConstants.FTP_DL_THROUGHPUT:
			return DriveHeaderConstants.SUMMARY_DL_INDEX;
		default:
			return null;
		}
	}

	private Double getAverageValueFromSummaryData(String summaryData, Integer indexKPI) {
		try {
			...
			logger.error("Unable to find the average value of kpi with summary data {} ,indexKPI {} ",summaryData,indexKPI);
		}
		return null;
	}
	 ...
	 */
	private DriveImageWrapper getDriveImageWrapperDataForImageGenearation(List<Integer> workorderId, Map<String, List<String>> map, List<KPIWrapper> kpiList, List<List<List<List<Double>>>> boundaryData, List<List<List<Double>>> driveRoute, List<SiteInformationWrapper> listOfSites) {
		...
		try {
			List<String[]> arlist = ReportUtil.convertCSVStringToDataList(getDriveData(workorderId, map));
			...
			return new DriveImageWrapper(arlist, DriveHeaderConstants.INDEX_LAT,
					DriveHeaderConstants.INDEX_LON, DriveHeaderConstants.INDEX_PCI, kpiList, siteDataList, boundaryData,driveRoute);
		} catch (Exception e) {
