...
public class ComponentSmallCell implements Component {
	private Logger logger = LogManager.getLogger(ComponentSmallCell.class);

	private PointF centerPoint;
	private BufferedImage img;

	public ComponentSmallCell(PointF pointF) {
		centerPoint = pointF;
	}
	...
	public void initializeImage() {
		try {
			 String path =ConfigUtils.getString(NVConfigUtil.INBUILDING_COMPONENT_IMAGE_PATH);
				img = ImageIO.read(new File(path+DrawFloorConstant.LTE_ICON));
		} catch (IOException e) {
		   logger.error("Exception initializeImage {}",Utils.getStackTrace(e));
		}
	}
