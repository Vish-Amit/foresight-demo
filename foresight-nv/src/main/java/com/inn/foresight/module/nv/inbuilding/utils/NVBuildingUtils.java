...
package com.inn.foresight.module.nv.inbuilding.utils;
...
public class NVBuildingUtils {

	/** The logger. */
	private static Logger logger = LogManager.getLogger(NVBuildingUtils.class);

	protected NVBuildingUtils() {
	}
	...
	public static MultipartEntityBuilder getMultiPartEntity(BuildingWrapper buildingWrapper, List<FileItem> fileItemList) {
		...
		try {
			...
			logger.error("Exception in getMultiPartEntity... {}", ExceptionUtils.getMessage(e1));
		}
	...
	public static String getFloorPlanRowKeyWithTemplateType(Integer unitId,String template) {
		...
		if(template!=null) {
			rowKey= StringUtils.reverse(StringUtils.leftPad(String.valueOf(unitId), InBuildingConstants.ROW_KEY_LENGTH,
				InBuildingConstants.PADDING_CHAR)).concat(String.valueOf(template.charAt(InBuildingConstants.FIRST_INDEX)));
		}else {
			rowKey= StringUtils.reverse(StringUtils.leftPad(String.valueOf(unitId), InBuildingConstants.ROW_KEY_LENGTH,
					InBuildingConstants.PADDING_CHAR));
		}
	...
	public static Put getFloorPlanPut(UnitInfoWrapper unitInfoWrapper, InputStream floorPlanStream,
		...
		Put floorPlanPut = new Put(Bytes.toBytes(NVBuildingUtils.getFloorPlanRowKeyWithTemplateType(unitInfoWrapper.getUnitId(),unitInfoWrapper.getTemplateType())));
		if (floorPlanStream != null) {
			floorPlanPut.addColumn(InBuildingConstants.COLUMN_FAMILY.getBytes(),
					...
					NVBuildingUtils.getBytesFromInputStream(floorPlanStream));
		}

		if (backgroundImageStream != null) {
			floorPlanPut.addColumn(InBuildingConstants.COLUMN_FAMILY.getBytes(),
					InBuildingConstants.COLUMN_BG_IMAGE.getBytes(),
					NVBuildingUtils.getBytesFromInputStream(backgroundImageStream));
		}
	...
	public static Put getFloorPlanPut(UnitInfoWrapper unitInfoWrapper, byte[] floorPlanStream,
		...
		Put floorPlanPut = new Put(
				Bytes.toBytes(NVBuildingUtils.getFloorPlanRowKeyWithTemplateType(unitInfoWrapper.getUnitId(),unitInfoWrapper.getTemplateType())));
	...
	/**
	 * Gets the test result put.
	 *
	 * @param unitInfoWrapper
	 *            the unit info wrapper
	 * @param ibTestResultWrapper
	 *            the ib test result wrapper
	 * @return the test result put
	 */
	public static Put getTestResultPut(UnitInfoWrapper unitInfoWrapper, IBTestResultWrapper ibTestResultWrapper) {
		Put testResultPut = new Put(Bytes.toBytes(NVBuildingUtils.getTestResultRowKey(ibTestResultWrapper)));

		testResultPut.addColumn(InBuildingConstants.COLUMN_FAMILY.getBytes(),
				InBuildingConstants.COLUMN_UNIT_ID.getBytes(), String	.valueOf(unitInfoWrapper.getUnitId())
																		.getBytes());

		testResultPut.addColumn(InBuildingConstants.COLUMN_FAMILY.getBytes(),
				InBuildingConstants.COLUMN_TEST_START_TIME.getBytes(),
				String	.valueOf(ibTestResultWrapper.getTestStartTime())
						.getBytes());
		...
		testResultPut.addColumn(InBuildingConstants.COLUMN_FAMILY.getBytes(),
				InBuildingConstants.COLUMN_TEST_END_TIME.getBytes(),
				String	.valueOf(ibTestResultWrapper.getTestStopTime())
						.getBytes());

		testResultPut.addColumn(InBuildingConstants.COLUMN_FAMILY.getBytes(),
				InBuildingConstants.COLUMN_TEST_TYPE.getBytes(), String	.valueOf(ibTestResultWrapper.getTestType())
																		.getBytes());

		testResultPut.addColumn(InBuildingConstants.COLUMN_FAMILY.getBytes(),
				InBuildingConstants.COLUMN_TEST_RESULT_JSON.getBytes(), new Gson()	.toJson(ibTestResultWrapper)
																					.getBytes());

		return testResultPut;
	}

	/**
	 * Gets the test result row key.
	 *
	 * @param ibTestResultWrapper
	 *            the ib test result wrapper
	 * @return the test result row key
	 */
	private static String getTestResultRowKey(IBTestResultWrapper ibTestResultWrapper) {
		return ibTestResultWrapper	.getDeviceId()
									.concat(String.valueOf(ibTestResultWrapper.getTestStartTime()));
